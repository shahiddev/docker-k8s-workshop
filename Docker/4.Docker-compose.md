...
We need to ensure our new .NET core project is targeting the .NET core 2.1 runtime (this is the long term release version)

Edit the web.csproj file and update the `<TargetFramework>` element to:
...
3. (option) upgrade the example to use .NET Core 3.1
4. (optional) Add a rabbitmq component to the application using the official rabbitmq image and extend the backend to send a message to rabbitmq
5. (optional)Explore using multiple compose-files per environment and overriding values (hint: look into docker-compose overrides)
